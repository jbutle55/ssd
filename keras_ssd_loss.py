...
class SSDLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        classification_loss = tf.cast(self.log_loss(
            y_true[:, :, :-12], y_pred[:, :, :-12]), dtype=tf.float32)
        # localization_loss = tf.to_float(self.smooth_L1_loss(y_true[:, :, -12:-8], y_pred[:, :, -12:-8]))  # Output shape: (batch_size, n_boxes)
        localization_loss = tf.cast(self.smooth_L1_loss(
            y_true[:, :, -12:-8], y_pred[:, :, -12:-8]), dtype=tf.float32)
        ...
        def f2():
            ...
            negatives_keep = tf.cast(tf.reshape(
                negatives_keep, [batch_size, n_boxes]), dtype=tf.float32)
